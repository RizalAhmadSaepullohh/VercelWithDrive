...
export default function AssessmentFlow() {
  ...
  // Pre-fill akun percobaan dikosongkan secara default agar tidak membingungkan pengguna asli
  useEffect(() => {
    setGoogleEmail("");
    setGoogleName("");
  }, []);
  ...
  const triggerGoogleSignIn = useCallback(async (email, name) => {
    if (!email || !name) {
    ...
    try {
      const res = await fetch(`/api/mahasiswa?email=${encodeURIComponent(email)}`);
      ...
      setMahasiswa(j?.mahasiswa || { id: email, email: email, nama: name });
      setNama(j?.mahasiswa?.nama || name);
  ...
  const onGoogleSignIn = useCallback(async (e) => {
    e?.preventDefault();
    await triggerGoogleSignIn(googleEmail, googleName);
  }, [googleEmail, googleName, triggerGoogleSignIn]);

  // Callback setelah login Google asli berhasil
  const handleGoogleCredentialResponse = useCallback(async (response) => {
    try {
      ...
      // Decode JWT token payload di sisi klien
      ...
      setAuthMsg(`Berhasil masuk sebagai ${name}. Memverifikasi akun...`);
      await triggerGoogleSignIn(email, name);
    } catch (err) {
      console.error("GSI handler error:", err);
      ...
      if (window.google?.accounts?.id) {
        ...
        try {
          window.google.accounts.id.initialize({
            client_id: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || "496026612324-amkc4m9go1f69pla9tfhe6uralas1dsu.apps.googleusercontent.com",
  ...
  // Re-render tombol Google Sign-In saat kembali ke halaman login
  useEffect(() => {
    if (step === -1 && window.google?.accounts?.id) {
      const btnDiv = document.getElementById("google-signin-btn");
      if (btnDiv) {
        window.google.accounts.id.renderButton(btnDiv, {
          theme: "outline",
  ...
  // Mengupdate/menyesuaikan nama responden setelah login
  const saveCustomName = useCallback(async () => {
    if (!tempName.trim()) return;
    try {
      setNameSaving(true);
      const targetEmail = mahasiswa?.email || googleEmail;
      if (!targetEmail) throw new Error("Email tidak ditemukan. Silakan login kembali.");
      ...
      if (!res.ok) {
        throw new Error("Gagal memperbarui nama di database.");
      }

      const j = await res.json();
      setMahasiswa(j?.mahasiswa);
      setNama(j?.mahasiswa?.nama);
  ...
  }, [mahasiswa, tempName, googleEmail]);
              ...
              {isEditingName ? (
                <div className="flex items-center gap-1 bg-blue-50 px-3 py-1 rounded-full border border-blue-100">
                  <input
                    ...
                    className="border border-blue-300 rounded px-2 py-0.5 text-xs text-gray-900 outline-none focus:ring-1 focus:ring-blue-500 bg-white max-w-[120px] sm:max-w-[180px]"
                    placeholder="Nama..."
                    disabled={nameSaving}
                    autoFocus
                    onKeyDown={(e) => {
                  ...
                  <button
                    ...
                    disabled={nameSaving}
                    className="px-1.5 py-0.5 text-[11px] font-bold text-green-700 hover:text-green-900 rounded hover:bg-green-100 disabled:opacity-50 transition"
                    title="Simpan"
                  >
                    ✓
                  </button>
                  <button
                    ...
                    disabled={nameSaving}
                    className="px-1.5 py-0.5 text-[11px] font-bold text-red-700 hover:text-red-900 rounded hover:bg-red-100 disabled:opacity-50 transition"
                    title="Batal"
                  >
                    ✕
                  </button>
                ...
                <div className="flex items-center gap-1.5 text-sm font-medium text-blue-900 bg-blue-50 px-3 py-1 rounded-full border border-blue-100 shadow-sm">
                  <span className="truncate max-w-[100px] sm:max-w-[200px]" title={mahasiswa?.nama}>{mahasiswa?.nama}</span>
                  <button
                    onClick={() => {
                      setTempName(mahasiswa?.nama || "");
                      setIsEditingName(true);
                    }}
                    className="text-blue-500 hover:text-blue-700 focus:outline-none text-[11px] hover:scale-105 transition"
                    title="Ubah nama panggilan Anda"
                  >
                    ✏️
                  </button>
                  <span className="text-blue-200 font-light select-none">|</span>
                  <span className="text-xs text-blue-700 font-semibold">Tugas {Math.max(1, Math.min(step, 6))}/6</span>
                </div>
              )}
            </div>
          )}
          ...
          <div className="relative overflow-hidden bg-white/80 backdrop-blur-md rounded-2xl border border-gray-100 shadow-xl p-8 max-w-lg mx-auto transition-all duration-300 hover:shadow-2xl">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-xs text-neutral-500 mt-1">
                Gunakan Akun Google Anda untuk masuk atau mendaftar otomatis.
              </p>
            </div>
            ...
            <div className="mb-6 flex flex-col items-center">
              ...
              <p className="text-[11px] text-gray-400 mt-2 text-center">
                *Menggunakan sistem autentikasi Google resmi.
              </p>
            </div>

            <div className="relative flex py-2 items-center mb-4">
              <div className="flex-grow border-t border-gray-200"></div>
              <span className="flex-shrink mx-4 text-[10px] font-bold uppercase tracking-wider text-gray-400">Atau Masuk Manual (Simulasi)</span>
              <div className="flex-grow border-t border-gray-200"></div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 mt-1">
                      *Diisi otomatis dari Google. Silakan ubah jika Anda ingin dipanggil dengan nama lain.
                    </p>
                  </div>
                  <div>
